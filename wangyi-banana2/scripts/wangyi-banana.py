        ...
        if isinstance(value, (list, tuple)):
            for item in value:
                if hasattr(item, 'read'):  # File-like object
                    cmd += ["-F", f"{key}=@{item.name}"]
                else:
                    cmd += ["-F", f"{key}={item}"]
        elif hasattr(value, 'read'):  # File-like object
            cmd += ["-F", f"{key}=@{value.name}"]
        else:
            cmd += ["-F", f"{key}={value}"]
    return subprocess.run(cmd, capture_output=True, text=True)
...
def api_request_with_backup(api_key: str, endpoint_suffix: str, payload: dict, 
    ...
    """Make API request with automatic backup host switching."""
    last_error = None
    
    for host_index, host in enumerate(BACKUP_HOSTS):
        url = f"{host}{endpoint_suffix}"
        headers = {
        ...
        # If all retries failed, try next host
        if host_index < len(BACKUP_HOSTS) - 1:
            time.sleep(2)
            continue
    ...
    if last_error:
        ...
        print(json.dumps({
            ...
            "message": "All backup hosts failed to respond"
...
def image_input_to_data_uri(img_input: str) -> str:
    """Convert image input into a data URI.

    Supported:
    - local file path
    - http(s):// URL (downloaded)
    - data:image/*;base64,...
    - b64:<base64> (wrapped into data:image/png;base64,...)
    """
    s = img_input.strip()
    ...
    return image_to_data_uri(s)


def parse_image_inputs(single_image: str | None, images_csv: str | None, max_count: int = 14) -> list[str]:
    """Parse image inputs from --image and --images.

    For Gemini models, each item may be: local file path, http(s) URL, data URI, or base64 string.
    For non-Gemini models, items must be local file paths.
    """
    image_items: list[str] = []
    if single_image:
        image_items.append(single_image.strip())
    if images_csv:
        image_items.extend([p.strip() for p in images_csv.split(",") if p.strip()])

    if len(image_items) > max_count:
        print(f"Error: too many images, max is {max_count}", file=sys.stderr)
        sys.exit(1)
...
def looks_like_base64(s: str) -> bool:
    import re
    # Heuristic: base64 strings usually contain only base64 alphabet + '='.
    if any(ch in s for ch in ("/", "\\", ":", "\n", "\r", "\t")):
        return False
    return re.fullmatch(r"[A-Za-z0-9+/=]{16,}", s) is not None


def image_input_to_gemini_url(img_input: str) -> str:
    """Convert an image input to a value usable in OpenAI-style `image_url.url`."""
    s = img_input.strip()
    if s.startswith("http://") or s.startswith("https://"):
        return s
    if s.startswith("data:"):
    ...
    if s.startswith("b64:"):
        ...
        if not looks_like_base64(b64):
            print("Error: invalid b64: content", file=sys.stderr)
            sys.exit(1)
        return f"data:image/png;base64,{b64}"
    ...
    p = Path(s)
    if not p.exists():
        print(f"Error: image input not supported or file not found: {s}", file=sys.stderr)
        sys.exit(1)
    return image_to_data_uri(str(p))
...
def cmd_generate_image(args):
    ...
    input_images = parse_image_inputs(args.image, args.images, max_count=14)

    is_gemini_model = "gemini" in model.lower()

    # Only use /v1/images/edits when nano-banana-edit is called with a single local file.
    is_edit = (
        (not is_gemini_model)
        and model == "nano-banana-edit"
        and len(input_images) == 1
        and Path(input_images[0]).exists()
    )
    
    if model == "nano-banana-edit" and not input_images:
    ...
    if is_edit:
        ...
        # For edit endpoint we only pass a single local file.
        ...
        if input_images:
            for img_input in input_images:
                image_url = image_input_to_gemini_url(img_input)
        ...
        # Image editing / composition using /v1/images/generations
        ...
        if input_images:
            payload["image_urls"] = [image_input_to_data_uri(img) for img in input_images]
...
def cmd_generate_video(args):
    ...
    # Validate parameters
    valid_models = ["sora-2", "grok-video-3"]
    if model not in valid_models:
        model = "sora-2"
        print(f"Warning: Invalid model, using sora-2", file=sys.stderr)
    
    # sora-2-pro 已移除：仅支持 10/15 秒。
    # 若用户手动传了 25 秒，这里自动折算为 15 秒。
    ...
    if duration == "25":
        duration = "15"
        print(f"Warning: 25s removed; using 15s", file=sys.stderr)

    # grok-video-3 is fixed to 15s per call
    if model == "grok-video-3" and duration != "15":
        if args.duration is not None:
            print(f"Warning: grok-video-3 only supports 15s, using 15s", file=sys.stderr)
        duration = "15"
...
""",
    ...
    parser.add_argument("--image", "-i", help="Input image (path/url/b64/data URI)")
    parser.add_argument("--images", help="Multiple input images (comma-separated, up to 14; path/url/b64/data URI)")
