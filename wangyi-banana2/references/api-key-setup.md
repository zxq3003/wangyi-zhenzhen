...
1. **注册账号**：[https://ai.t8star.cn/register?aff=253d8a45349](https://ai.t8star.cn/register?aff=253d8a45349)
2. **添加令牌**：左侧「令牌」→「添加令牌」（名字随便写，其它默认）→ 提交
3. **充值算力**：左侧「钱包」→ 充值（汇率约 5 刀=6.75 RMB，最低 5 刀起充）
4. **复制 API Key**，妥善保管

## 快速配置：让 OPENCLAW 帮你写入

在 OPENCLAW 中发送：

> 我现在发送 APIKEY，帮我配置好。**注意是在 openclaw.json 添加，不可覆盖或删除原来里面的配置参数。**
