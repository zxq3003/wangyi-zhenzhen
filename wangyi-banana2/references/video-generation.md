...
## 支持的模型
- **sora-2** - 标准 SORA2 视频生成模型（**default 分组**，10s/15s 按次 0.1）；更准确、更逼真、更可控；支持对话/音效（可在提示词里写要求）
- **grok-video-3** - 固定 15s/次（费用 0.5）

> ⚠️ **sora-2 必须使用 default 分组**。严禁调用 SORA-VIP、OPENAI官-优质、ORIGIN —— 这些渠道按秒计费，贵到离谱！

## 视频参数

### 时长选项
- `10` - 10 秒视频（`sora-2`）
- `15` - 15 秒视频（`sora-2`、`grok-video-3`）
...
### 文生视频
1. **选择模型** - 根据需求选择 `sora-2` 或 `grok-video-3`
2. **输入提示词** - 详细描述您想要的视频内容；如果你用 Chat 风格，建议在提示词开头加参数要求（如 `(竖屏/横屏,15s,高清)`）
3. **设置参数** - 选择时长、比例等
4. **生成视频** - 等待 AI 处理完成（通常需要几分钟）

### 图生视频

1. **上传参考图片** - 选择一张参考图片（脚本当前支持单张；如需多图参考可在后续扩展）
2. **输入提示词** - 描述视频的动作和效果
3. **设置参数** - 选择时长、比例等
4. **生成视频** - 等待 AI 处理完成
...
- 视频生成通常需要 3-10 分钟
- grok-video-3 固定 15 秒/次；sora-2 支持 10/15 秒
- 视频生成过程中会显示进度
- 建议使用高清模式获得更好效果
- 角色客串功能需要先创建角色
